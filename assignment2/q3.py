...
atmosfearVisitsDataFrame = pd.DataFrame(atmosfearVisits, columns=['Atmosfear attendence during the day'])
atmosfearVisitsDataFrame.insert(0, 'time', ["t+"+str(i) for i in range(192)])
print(atmosfearVisitsDataFrame.values.tolist())
lines = atmosfearVisitsDataFrame.plot.line(x='time', y='Atmosfear attendence during the day')
plt.show()
c.close()
